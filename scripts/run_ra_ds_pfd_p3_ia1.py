"""Launch IA-1 fixed arms through the existing public training CLI."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Run IA-1 fixed selected-only RA-DS-PFD arms through the public "
    ...
    selectors.add_argument("--all", action="store_true", help="run both IA-1 arms in order")
...
def base_run_id(value: str | None) -> str:
    return validate_run_id(
        value or datetime.now().strftime("ra-ds-pfd-p3-ia1-%Y%m%d-%H%M%S")
    )
        ...
        plan.append(
            {
                ...
                "selection_mode": ia_config["selection_mode"],
                "selected_candidates": list(ia_config["selected_candidates"]),
                "effective_candidate_count": len(ia_config["selected_candidates"]),
                ...
                "resolved_model_config_source": (
                    f"{SUITE_PATH} -> frozen R2 -> pfd3_ia_fixed({variant})"
                ),
...
def resolved_model_document(
    ...
    if variant not in VARIANT_IDS:
        raise ValueError(f"unsupported RA-DS-PFD IA-1 variant: {variant}")
...
def execute_plan(args: argparse.Namespace, plan: list[dict[str, Any]]) -> int:
    ...
    temporary_root = Path(tempfile.mkdtemp(prefix="ra_ds_pfd_p3_ia1_"))
    try:
        for item in plan:
            ...
            try:
                ...
                print(
                    f"Launching {variant} with fixed candidates "
                    f"{item['selected_candidates']}: {json.dumps(command, ensure_ascii=False)}",
                    flush=True,
                )
                completed = subprocess.run(command, cwd=ROOT, check=False)
                if completed.returncode != 0:
                    print(
                        f"RA-DS-PFD IA-1 suite stopped after {variant} "
        ...
        json.dumps(
            {
                ...
                "suite": "ra_ds_pfd_p3_ia1",
                "base_variant": BASE_VARIANT,
                "pfd_mode": "pfd3_ia_fixed",
                "variants": plan,
                "plan": plan,
            },
            ensure_ascii=False,
