"""Node-shared adapter around Time-Series-Library TSMixer."""
...
_CONFIG_FIELDS = {"d_model", "e_layers", "dropout"}


class TSMixer(NodeSharedForecastModel):
    """Apply one shared upstream TSMixer instance independently per node."""
    ...
    def forward_node_chunk(
        ...
        x = self._node_chunk_x(inputs, node_start, node_end, model_name="TSMixer")
...
def _validate_config(model_config: dict[str, Any]) -> None:
    validate_time_series_library_config_fields(
        model_config, model_name="TSMixer", fields=_CONFIG_FIELDS
    )
    d_model = model_config["d_model"]
    e_layers = model_config["e_layers"]
    dropout = model_config["dropout"]
    if not isinstance(d_model, int) or isinstance(d_model, bool) or d_model < 1:
        raise ValueError("TSMixer d_model must be a positive integer")
    if not isinstance(e_layers, int) or isinstance(e_layers, bool) or e_layers < 1:
        raise ValueError("TSMixer e_layers must be a positive integer")
    if isinstance(dropout, bool) or not isinstance(dropout, (int, float)) or not 0.0 <= float(dropout) < 1.0:
        raise ValueError("TSMixer dropout must be in [0, 1)")


def build_model(model_config: dict[str, Any], data_info: DataInfoView) -> TSMixer:
    _validate_config(model_config)
    validate_time_series_library_data_info(data_info, model_name="TSMixer")
    source_root = resolve_time_series_library_source_root(
        data_info.project_root, model_name="TSMixer"
    )
    upstream_class = load_time_series_library_model_class(
        "TSMixer", source_root=source_root
    )
    return TSMixer(
