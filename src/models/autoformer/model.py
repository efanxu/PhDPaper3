"""Node-shared adapter around the official Time-Series-Library Autoformer."""
...
class Autoformer(NodeSharedForecastModel):
    """Apply one shared official Autoformer instance independently per node."""
    ...
    def forward_node_chunk(
        ...
        x = self._node_chunk_x(inputs, node_start, node_end, model_name="Autoformer")
...
def _validate_config(model_config: dict[str, Any], *, lookback: int) -> None:
    validate_time_series_library_config_fields(
        model_config, model_name="Autoformer", fields=_CONFIG_FIELDS
    )
    positive_fields = (
        "d_model", "n_heads", "d_ff", "e_layers", "d_layers", "factor", "moving_avg", "label_len"
    )
    if not all(
        ...
        raise ValueError("Autoformer integer dimensions and counts must be positive")
    if model_config["d_model"] % model_config["n_heads"]:
        raise ValueError("Autoformer d_model must be divisible by n_heads")
    if model_config["label_len"] > lookback:
        raise ValueError("Autoformer label_len must not exceed lookback")
    if model_config["moving_avg"] % 2 == 0:
        raise ValueError("Autoformer moving_avg must be a positive odd integer")
    dropout = model_config["dropout"]
    if isinstance(dropout, bool) or not isinstance(dropout, (int, float)) or not 0.0 <= float(dropout) < 1.0:
        raise ValueError("Autoformer dropout must be in [0, 1)")
    if model_config["activation"] not in {"relu", "gelu"}:
        raise ValueError("Autoformer activation must be relu or gelu")
    if model_config["embed"] not in {"timeF", "fixed", "learned"}:
        raise ValueError("Autoformer embed must be timeF, fixed or learned")
    if model_config["freq"] not in {"h", "t", "s", "m", "a", "w", "d", "b"}:
        raise ValueError("Autoformer freq is not supported by Time-Series-Library")


def build_model(model_config: dict[str, Any], data_info: DataInfoView) -> Autoformer:
    validate_time_series_library_data_info(data_info, model_name="Autoformer")
    _validate_config(model_config, lookback=int(data_info.lookback))
    source_root = resolve_time_series_library_source_root(
        data_info.project_root, model_name="Autoformer"
    )
    upstream_class = load_time_series_library_model_class(
        "Autoformer", source_root=source_root
    )
    return Autoformer(
