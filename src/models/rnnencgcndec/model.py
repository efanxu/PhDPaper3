"""Formal ``tsl`` RNN encoder/GCN decoder adapter."""
...
def _validate_config(model_config: dict[str, Any]) -> None:
    if not isinstance(model_config, dict):
        raise ValueError("RNNEncGCNDec model config must be a mapping")
    unknown = sorted(set(model_config) - _CONFIG_FIELDS)
    missing = sorted(_CONFIG_FIELDS - set(model_config))
    if unknown:
        raise ValueError(f"RNNEncGCNDec model config has unknown field: {unknown[0]}")
    if missing:
        raise ValueError(f"RNNEncGCNDec model config is missing field: {missing[0]}")
    for name in ("hidden_size", "rnn_layers", "gcn_layers"):
        value = model_config[name]
        if not isinstance(value, int) or isinstance(value, bool) or value < 1:
            raise ValueError(f"RNNEncGCNDec {name} must be a positive integer")
    for name in ("rnn_dropout", "gcn_dropout"):
        value = model_config[name]
        if isinstance(value, bool) or not isinstance(value, (int, float)) or not 0.0 <= float(value) < 1.0:
            raise ValueError(f"RNNEncGCNDec {name} must be in [0, 1)")
    if model_config["cell_type"] not in {"gru", "lstm"}:
        raise ValueError("RNNEncGCNDec cell_type must be gru or lstm")
    if model_config["activation"] not in {
        ...
        raise ValueError("RNNEncGCNDec activation is not supported by TSL")
...
def _validate_node_ids(data_info: DataInfoView) -> None:
    ...
    if not node_ids or len(node_ids) != int(data_info.num_nodes):
        raise ValueError("RNNEncGCNDec requires public data node_ids aligned to every graph node")
    if len(set(node_ids)) != len(node_ids):
        raise ValueError("RNNEncGCNDec node_ids must not contain duplicates")


class RNNEncGCNDec(ForecastModel):
    """Normalize the installed TSL RNN/GCN model to ``(B, N, H)``."""
    ...
    def __init__(
        ...
        *,
        ...
        self.upstream = upstream_model(
            ...
            rnn_dropout=float(model_config["rnn_dropout"]),
            gcn_dropout=float(model_config["gcn_dropout"]),
            horizon=self.horizon,
            cell_type=str(model_config["cell_type"]),
            activation=str(model_config["activation"]),
        )

    def forward(self, inputs: ModelInput) -> torch.Tensor:
        if not isinstance(inputs, ModelInput):
            raise TypeError("RNNEncGCNDec expects ModelInput")
        if any(
            value is not None
            for value in (
            ...
            raise ValueError("RNNEncGCNDec receives its shared graph at construction and accepts history x only")
        x = inputs.x
        if x.ndim != 4:
            raise ValueError("RNNEncGCNDec expects x with shape (B, L, N, C)")
        ...
        if (steps, nodes, channels) != expected_input:
            raise ValueError(
                "unexpected RNNEncGCNDec input shape: "
                f"{tuple(x.shape)}; expected (*, {expected_input[0]}, {expected_input[1]}, {expected_input[2]})"
            )
        if not torch.isfinite(x).all():
            raise FloatingPointError("RNNEncGCNDec input contains NaN or Inf")
        ...
        if tuple(upstream_output.shape) != expected_output:
            raise ValueError(
                "RNNEncGCNDecModel output must have shape "
                f"{expected_output}, got {tuple(upstream_output.shape)}"
            )
        if not torch.isfinite(upstream_output).all():
            raise FloatingPointError("RNNEncGCNDecModel output contains NaN or Inf")
...
def build_model(model_config: dict[str, Any], data_info: DataInfoView) -> RNNEncGCNDec:
    _validate_config(model_config)
    _validate_node_ids(data_info)
    if data_info.project_root is None:
        raise ValueError("RNNEncGCNDec requires project_root metadata for the shared graph resource")
    try:
        from tsl.nn.models.stgn import RNNEncGCNDecModel
    except ImportError as exc:
        raise ImportError(
            "RNNEncGCNDec requires the formal tsl.nn.models.stgn.RNNEncGCNDecModel in the tsl runtime environment"
        ) from exc
    ...
    return RNNEncGCNDec(
        upstream_model=RNNEncGCNDecModel,
        data_info=data_info,
