...
class _ValidationCapabilityToy(_ValidationToy):
    ...
    def to(self, *args, **kwargs):  # type: ignore[no-untyped-def]
        # The test simulates the CUDA device selector without allocating CUDA tensors.
        del args, kwargs
        return self
...
def test_shape_validation_uses_shared_training_algorithm_context(
    monkeypatch: pytest.MonkeyPatch, capability: bool
) -> None:
    monkeypatch.setenv("PYTHONHASHSEED", "2026")
    monkeypatch.setenv("CUBLAS_WORKSPACE_CONFIG", ":4096:8")
    info = DataInfoView(
    ...
    toy = _ValidationCapabilityToy(capability)
    captured: list[bool] = []

    class SpyPolicy:
        def as_dict(self):
            return {
                "amp_configured": False,
                "amp_effective": False,
                "amp_dtype": "float16",
    ...
    def fake_executor(model, batches, **kwargs):  # type: ignore[no-untyped-def]
        captured.append(torch.are_deterministic_algorithms_enabled())
        batch = batches[0]
        with kwargs["autocast"]():
            prediction = model(batch.model_input())
            loss = prediction.square().mean()
    ...
    original_randn = validation.torch.randn

    def cpu_randn(*shape, **kwargs):  # type: ignore[no-untyped-def]
        kwargs = dict(kwargs)
        kwargs.pop("device", None)
        return original_randn(*shape, **kwargs)

    monkeypatch.setattr(validation, "_choose_device", lambda value: torch.device("cuda"))
    monkeypatch.setattr(validation.torch, "randn", cpu_randn)
    monkeypatch.setattr(validation.torch.cuda, "reset_peak_memory_stats", lambda device: None)
    monkeypatch.setattr(validation.torch.cuda, "empty_cache", lambda: None)
    monkeypatch.setattr(validation, "_memory_snapshot", lambda device: {})
    monkeypatch.setattr(validation, "load_data", lambda config, project_root: (None, info))
    monkeypatch.setattr(validation, "load_model_config", lambda path: {})
    monkeypatch.setattr(validation, "build_model", lambda name, config, data_info: toy)
    monkeypatch.setattr(validation, "resolve_precision_policy", lambda **kwargs: SpyPolicy())
    monkeypatch.setattr(validation, "execute_training_backward", fake_executor)
